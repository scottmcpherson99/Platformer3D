...
#include "Core/AbilitySystem/AsyncTasks/AsyncTaskCooldownChanged.h"
...
{
	// create the listen object and set up params
	...
	// if ability system component is not valid or there are no tags, end the task
...
{
	// check if ability system component is still valid
	if (IsValid(ASC))
	{
		// remove all gameplay effects
		ASC->OnActiveGameplayEffectAddedDelegateToSelf.RemoveAll(this);

		// get cooldown tag array
		TArray<FGameplayTag> CooldownTagArray;
		CooldownTags.GetGameplayTagArray(CooldownTagArray);

		// remove all gameplay tag
		for (FGameplayTag CooldownTag : CooldownTagArray)
		{
